...
enum_start (gegl_imagegradient_output)
   enum_value (MY_MAG, "magnitude", N_("Magnitude"))
   enum_value (MY_DIR, "direction", N_("Direction"))
   enum_value (MY_BOTH,      "both",      N_("Both"))
enum_end (MyImageGradientOutput)

property_enum (output_mode, _("Output mode"),
               MyImageGradientOutput, gegl_imagegradient_output,
               0)
...
#define GEGL_OP_NAME         my_image_gradient
      ...
      for (x = 1; x < row_rect.width - 1; x++)
        {
          ...
          if (o->output_mode == 0)
          ...
          else
           {
              ...
              gfloat direction = atan2 (dx[max_index], dy[max_index]); 
              ...
              if (o->output_mode == 1)
                ...
                {
                  // g_debug ("%s direction = %f", G_STRFUNC, direction);
                  row4[(x-1) * n_components] = magnitude[max_index];
